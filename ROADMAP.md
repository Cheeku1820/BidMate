...
- **Object storage with tenant-scoped keys — built** (B1, [`docs/specs/documents-stored.md`](docs/specs/documents-stored.md)): MinIO locally and S3 in deployment behind one `BlobStore` interface, keys built from the owning org and project, a `documents` row per upload with its hash and storage key, within-project duplicates refused by the database, every mutation audited. **A retention policy is not built**, and the gaps are specific: a `put` that succeeds ahead of a commit that fails, and every file under an archived project (no route deletes a project, so its blobs are unreachable forever), leave storage nothing sweeps. Deleting a document removes its row first and commits, then its file; a storage failure leaves an orphan blob nothing references, never a row that points at nothing. A reaper needs a policy first — what an archived project's files are kept for, and for how long — and that is a product call before it is code.
...
- **Prioritization by bid date — not built.** Bid deadlines are hard deadlines. An estimator whose sheets sit behind a competitor's 400-sheet hospital set at 4pm the day before a bid has already churned. Across projects every ready job of a kind is claimed oldest-first regardless of whose bid is due sooner; the only priority today is takeoff work over renders.
