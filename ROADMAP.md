...
- **Object storage with tenant-scoped keys — built** (B1, [`docs/specs/documents-stored.md`](docs/specs/documents-stored.md)): MinIO locally and S3 in deployment behind one `BlobStore` interface, keys built from the owning org and project, a `documents` row per upload with its hash and storage key, within-project duplicates refused by the database, every mutation audited. **A retention policy is not built**, and the gaps are specific: a `put` that succeeds ahead of a commit that fails, a `delete` whose row goes before its blob does, and every file under an archived project (no route deletes a project, so its blobs are unreachable forever) all leave storage nothing sweeps. A reaper needs a policy first — what an archived project's files are kept for, and for how long — and that is a product call before it is code.
