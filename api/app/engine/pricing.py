...
book, labor hours from the catalog, labor cost at the firm's blended rate
(the labor rate that already lives on the Company settings screen). The
engine stops at total direct cost -- markup, overhead, and profit are the
estimator-owned layer and no agent proposes them (invariant 13).
...
def price_item(item: ClassifiedItem, labor_rate: float) -> PricedItem:
    ...
    material = round(cat.material_cost * item.quantity, 2)
    hours = round(cat.labor_hours * item.quantity, 2)
