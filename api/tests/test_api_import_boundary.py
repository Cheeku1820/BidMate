...
The same line keeps `app.worker` out entirely: it is the only package
that opens a PDF, and the only one that holds the market sources' keys
and makes their calls (`worker/market_sources.py`). An API process that
imported it would have both.
...
def test_the_api_process_does_not_import_the_pdf_pipeline_or_the_worker():
    """A bug in counting.py, or a missing pymupdf wheel, must not stop
    the API container booting, and the market sources' HTTP clients must
    never be loaded where a request is served. Run in a subprocess so
    this test's own imports don't pollute it."""
    code = (
    ...
    banned = [m for m in imported if m in _PDF_PIPELINE_MODULES or m.startswith("app.worker")]
    assert not banned, f"app.main pulled in the PDF pipeline or the worker: {banned}"
