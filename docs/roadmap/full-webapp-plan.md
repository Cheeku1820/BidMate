...
**History as of B3.** No longer true on `feat/drawing-behind-markers`: `PlanDrawing.jsx` is deleted, and `TileLayer.jsx` draws the rendered tile pyramid under the markers at the page's true paper aspect (B3, [`docs/specs/drawing-behind-the-markers.md`](../specs/drawing-behind-the-markers.md)). A sheet still awaiting its render, or one whose render failed, falls back to the old blank-paper treatment with its render state on it — so "the canvas has no drawing behind it" is now the exception, not the default. The evidence crop remains the second witness, not the only one. Calibration now has the real page to click against, but the two-click distance it computes (`paperDistanceToPoints`) is still discarded — `Workspace.jsx`'s `onCalibrate` opens the scale modal and ignores the argument, so calibration is visually real but not yet wired to a number. Left as written above because it is the audit that motivated the B3 spec; do not read it as the current state of `main`, where none of Phase A–B has merged yet either.
...
`llm.py` returns material cost, labor hours, a location labor rate and a material factor from one Claude call. Without a key, every labor and material row shows *Missing information* by design (`docs/specs/labor-material-pricing.md`, "no hardcode"). The static `catalog.py` price book is eleven items with placeholder figures. There are no supplier quotes, no price-book import. The four Excel workbooks are a real firm's unit labor hours and unit material costs across ~150 line items — the first real pricing source this project has had.

**Correction, checked against the code.** "There is no assembly expansion in the shipped path" was already wrong when this was written on 2026-09-13: the model-classified path has priced a device's assembly since `a5b8785` ("Give the model-classified path an assembly, or none at all", 2026-09-03, on `main` well before this audit), and the deterministic path always has, through `pricing.price_item`. On `feat/engine-behind-api` that logic lives in `engine/rows.py` (`resolve_assembly_parent`/`_row_from_spec` for the model path, `pricing.price_item` for the deterministic one), both calling `assemblies.expand` — every device is priced with its box, plate, wire, and conduit, run by the worker, not only the CLI. What is genuinely missing is the dedicated **Assemblies workspace** (per-catalog-item editing, company overrides, a waste factor) — see the table below and Phase C 4.4.
...
| Assemblies | disabled | — | Workspace unbuilt (per-catalog-item editing, company overrides, waste factor). "Nothing in the app reads it" was already wrong at this audit — `assemblies.py` has priced every device's box, plate, wire and conduit since before this document was written, on both classification paths (`engine/rows.py` on `feat/engine-behind-api`; `engine/estimate.py`/`pricing.py` on `main`) |
...
- [x] **Page rendering at ingest.** Built (B3, [`docs/specs/drawing-behind-the-markers.md`](../specs/drawing-behind-the-markers.md)): the `read` job queues one `render` job per detected sheet, which cuts the page into a 512 px tile pyramid — level 0 fits one tile, each further level doubles until 72 · scale reaches 150 dpi — plus a 240 px thumbnail, into the blob store under a content-hashed prefix. `GET /api/sheets/{id}/tiles/{z}/{x}/{y}.png` and `.../thumb.png` serve them, cached `private, max-age=604800, immutable` (the one carve-out from the no-shared-caching middleware, since the URL embeds the content hash). This is what puts the drawing back behind the markers.
...
**Done when:** an estimator uploads a set, closes the tab, comes back, watches sheets complete one by one, and opens the review workspace with the real drawing rendered behind the markers. True as of B3 — every clause in that sentence is built, including the rendered drawing.
...
### 4.4 Pricing
- [x] **Assembly expansion in the app path**, not just the CLI: device → box, plate, ring, whip, conductors, conduit per `assemblies.py`. **Built** — and, per the correction in §0.1 fact 4 and §0.2 above, already built before this plan was written on 2026-09-13, not something B2 newly landed; B2's contribution was moving the call site into `engine/rows.py`, run from the sheet job, alongside everything else the module split touched. **Waste applied to material only, inputs stored and product derived is not built** — `assemblies.py`/`pricing.py` carry no waste factor today; that half of this item is still open (`docs/product/mvp-approach.md` §4.1).
