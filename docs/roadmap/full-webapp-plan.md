...
- [ ] **A deployment target.** Pick one (Fly.io / Render / a single VM with Compose) and stand up API + Postgres + a worker. The current setup — engine on the developer's laptop at `:8100`, web via Vite dev server — cannot be handed to anyone. Includes secrets management (`ANTHROPIC_API_KEY` today is an env var on a laptop), TLS, and a real `VITE_API_BASE`.
...
Replaces browser → `:8100` with browser → API → storage → worker → store. Everything in §0.1 fact 1.

- [ ] **Document storage.** `documents` (project, filename, type, sha256, size, uploaded_by, status) and `document_pages` (document, page index, width/height pt, tier, render key). Object storage with tenant-scoped keys — MinIO locally, S3 in deployment. **Decide the cross-tenant deduplication policy before writing this** (`ROADMAP.md` "decisions to make before building"): content-addressed dedup across orgs leaks that a competitor is bidding the same job. Recommendation: hash for duplicate detection *within* a project only; never share blobs across orgs.
- [ ] **Upload endpoint.** Signed-URL direct-to-storage multipart upload for 100–500 MB sets; the API records the document when the upload completes. Duplicate-in-project, password-protected, corrupt, and unsupported-type outcomes each get their own state on screen C (spec §5 C).
- [ ] **Job queue and worker.** Start with a `jobs` table (project, document, page, stage, status, attempts, error) and one worker process polling it — no Redis yet. One job per sheet so a sheet fails alone (spec §5 E). Retries, dead-letter after N, per-tenant concurrency cap. The seam already exists: `engine/pipeline.py` is called synchronously today.
- [ ] **Move the engine behind the API.** `estimate_service.py` becomes the worker's entry point, not an HTTP service. `engineClient.js` goes away. The worker runs in its own container with memory/time limits — PDF parsers are an RCE surface and this is the minimum sandbox.
- [ ] **Page rendering at ingest.** Render every sheet to a PNG (or a small tile pyramid for E-size) into storage; serve through an authenticated API route. This is what puts the drawing back behind the markers.
- [ ] **Processing status for real.** `GET /projects/{id}/processing` returns per-sheet stage from the jobs table; screen E polls it; "you can leave this page" becomes true. Completed sheets are reviewable while others run.
- [ ] **Confirm drawings (screen D) writes back.** Include/exclude, discipline correction, revision, and scale corrections persist to `document_pages` and gate which pages get jobs. Today none of that survives.
- [ ] **Metering events.** Emit `sheet_processed` with tenant, project, page, tier, duration from the worker. Not billed, but `ROADMAP.md` 3.1 is right that this cannot be retrofitted.

**Done when:** an estimator uploads a set, closes the tab, comes back, watches sheets complete one by one, and opens the review workspace with the real drawing rendered behind the markers.
