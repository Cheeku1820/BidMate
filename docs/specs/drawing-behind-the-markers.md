...
- **Levels.** Level 0 is the scale at which the page's long edge fits one 512 px tile. Each level doubles. Levels continue until the page reaches **≥ 150 dpi** on its long edge (`points × 150/72 ≤ 512 × 2^z`) — checked against `tiles.levels_for`, ARCH D (24 × 36 in, long edge 2592 pt) reaches 227.6 dpi at z = 4 and ARCH E (36 × 48 in, long edge 3456 pt) reaches 170.7 dpi also at z = 4; both land on the same level because each doubling roughly doubles dpi, so the one-size-larger E sheet needs the same level D does once D has already overshot 150 dpi at z = 4. `max_zoom` is the last level rendered.
...
Where the plan's sketches met real code, several points came out different from how §3–§6 above describe them. None change the design; each is recorded here rather than silently in the diff.
...
- **`levelFor` now takes `paper` and compares against its long edge, DPR-adjusted.** Originally compared the on-screen width against `SHEET_W` (1000) alone, which is exact for a landscape or square page but a step coarser than `max_zoom` actually supports on a portrait page (the long edge there is the height, not the normalized-to-1000 width). Fixed in the whole-branch review: `levelFor(viewScale, maxZoom, paper)` compares `512 · 2^z` against `Math.max(paper.w, paper.h) * viewScale * (window.devicePixelRatio || 1)`, matching `gridFor`'s long-edge definition and additionally accounting for device pixel ratio — a high-DPI screen needs a finer level to show 512 real device pixels per tile, not 512 CSS pixels. Covered in `TileLayer.test.jsx`.
...
- **Upload storage errors are classified the same way download storage errors are.** §4 says "transient storage errors retry as every job does," but the render job's `store.put` loop originally bypassed the classification `blob_to_tempfile` applies to `store.open` — any storage exception there, transient or not, surfaced as an unhandled exception and landed as a terminal `RENDER_FAILED` on the first attempt. Fixed in the whole-branch review: the classification moved into a shared `blobs.storage_errors(filename)` context manager (`BlobNotFound` → `Terminal`, `OSError`/`ConnectionError`/a botocore exception → `Transient(copy.UNAVAILABLE)`), used by both `blob_to_tempfile`'s download and the render job's upload loop. Covered in `tests/test_worker_render.py`.
- **Migration 0022's `downgrade()` deletes any `render` job rows before recreating `ck_jobs_kind`'s three-kind form.** Without the delete, a downgrade after even one render job had run would fail outright — `create_check_constraint` validates existing rows, and a `kind = 'render'` row cannot satisfy a constraint that no longer lists `render`. Render job rows are disposable queue entries, not audit history (the append-only `actions` table is that), so deleting them on the way down loses nothing the migration is responsible for preserving. Covered in `tests/test_migration_0022.py`.
