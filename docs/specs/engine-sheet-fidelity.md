...
| Pulte Sagebriar | `PLANS/…Electrical.pdf` | 10 | 0 | `E-001` … `E-801` (`E-010`, `E-210`, `E-700`); the `EX1`, `EX51`, `EF-1` tokens on its pages are luminaire-type and fan tags in the same shape, not sheet numbers | **0** |
...
1. For each of the four edge strips (outer 18 % of width or height), find the type size of the largest *sheet-number-shaped* token it holds — any discipline's, `[A-Z]{1,3}-?\d{1,3}(\.\d{1,2})?` — where a token's type size is `min(bbox width, bbox height)` rounded to the point: its extent across its reading direction, whichever way the word runs, so a rotated number cell measures the same as an upright one and float noise never orders two tokens set in one face — and count *distinct* sheet-number-family tokens (§2.5) (a revision table repeating one other sheet's number three times must not outscore the strip holding the actual number cell) plus the labels `SHEET`, `DRAWN`, `CHECKED`, `DATE`, `PROJECT`, `REVISION`.
...
- On one rotated Unalaska plan page, a hand-chosen device tag's visual coordinate, when a small square around it is rendered from the page's rotated pixmap, contains that tag's glyph (assert via `page.get_textbox` on the derotated clip returning the tag). This is the test that proves markers will sit on the drawing. The 270° half of this was to come from a TSC Nutrition page, and cannot: every TSC electrical page is 270° and carries no words at all (its text is outlined to paths), so there is no tag to read back. Two synthetic tests in `test_engine_documents.py` stand in — a 180° and a 270° page built by the `_sheet()` helper, asserting number, title, kind, the counting region giving up the correct edge, and every placement Counting emits reading back as its own tag through the derotated clip (the same clip in the raw frame reads nothing).
