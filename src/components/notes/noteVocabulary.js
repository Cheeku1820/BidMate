 ...
 *  *not* been applied.
 *
 *  This is banner state, not what a re-run reads: the worker reads
 *  every standing context note from the database on each run, applied
 *  or not (api/app/worker/classify_job.py), so an earlier note is never
 *  silently dropped by a later apply. The applied stamp only records
 *  "carried in at least once". */
